...
## Forward-campaign implementation (built Sept 24, 2026 — no results yet)

Built against the frozen `docs/forward-campaign.md` (incl. Amendment A1, committed by Shrikant before any testing). Facts only — nothing below has been run on real data.

- **Data:** `EtfUniverse` = NIFTYBEES, BANKBEES, ITBEES, PHARMABEES, PSUBNKBEES, AUTOBEES (not membership-gated). `download` fetches them after the stock universe; an ETF with no token/no data/fetch error is a loud warning, an ETF with non-today data is STALE (exit 2). `CostModel.etf()`: STT 0.001% sell-only, DP ₹15.34 (same as equity, Amendment A2), same txn/SEBI/GST/stamp/slippage; chosen by the frozen list because Kite files ETFs as `EQ`.
- **Strategies (frozen, `signal/strategies/`):** `IndexMeanReversionStrategy` IMR-v1, `SectorRotationStrategy` ROT-v1, `VolRegimeStrategy` VRS-v1, shared `DisasterStop` (entry fill − 2.5×ATR(14), ATR as of the signal day). Implementation choices where the text needed mechanics: holding periods count the data's own bars (`Indicators.barsSince`), not Position's weekday approximation; ROT completes a rotation one day after the review (sleeve cash is tied up until the exit fills) — the day after a review, a flat sleeve enters the review-day leader; VRS is state-based ("invested iff"), so a flat sleeve enters whenever the regime is on (first run, after a stop).
- **Sleeves:** `RiskManager.sleeveProfile()` (A1: max position 100% of the sizing equity, no stop-distance cut; rails unchanged); additive `approve(..., sizingEquity, railEquity, railPeak, railWeekly)` overload — the no-arg profile and 5-arg `approve` are unchanged. Journal schema v1 (`PRAGMA user_version`): `sleeve` column on signals/orders, equity_daily keyed (sleeve, date), paper_positions keyed (sleeve, symbol), new `live_orders` table; pre-existing rows migrate to sleeve `paper`. `SleeveCycle` runs all sleeves; funded sleeves size on min(sleeve equity, sleeve capital), the kill switch/weekly pause judge the TOTAL account (funded sleeves + unallocated buffer, namespace `TOTAL`); funded sleeves fill the full ordered quantity on gap-ups (ledger = broker; the ₹2k buffer absorbs the overdraft). Breakout shadow = `BreakoutStrategy(50, 1.5, 3.0, 10)` (the exam config per §3 — NOT `breakout2`), default RiskManager, equity costs, membership-gated, virtual ₹50,000, own rails. A second `cycle` on the same date re-evaluates nothing. `PaperTrader` now only fills orders created on earlier evenings and doesn't queue an order identical to one still pending.
- **Executor:** `OrderExecutor` (DRY_RUN default via `live.enabled=false`) → `BrokerGateway`/`KiteBrokerGateway`: CNC AMO MARKET, market_protection −1, tag = "ST"+18 hex of SHA-256(date|sleeve|symbol|side); reads today's order book before placing (skip known tags; unreadable book → place nothing); a placement error is reconciled, never retried; BUYs refused under kill switch/weekly pause or above sleeve capital / `capital.total`; exits always go out; 1s between placements. REFUSED/FAILED orders are cancelled in the sleeve ledger. `live` exits 3 if any order FAILED.
- **Config keys:** `live.enabled` (false), `capital.total` (50000), `sleeve.imr|rot|vrs` (16000).
- **Commands / run configs:** `cycle` (14), `live` (15), `live reset-peak [sleeve]`, `backtest imr|rot|vrs [start] [end]` (16–18: §7 sanity checks — ETF-only universe, sleeve profile, ETF costs, sleeve capital; rails act on the sleeve itself in these backtests). Run each ONCE; publish regardless of outcome.
- **Known gaps:** ledger fills are modelled (next open + CostModel), not read back from Kite — true fill prices for §9 still need a fills reconciliation; a broker-side rejection AFTER placement (e.g. at the open) isn't detected, so check the Kite order book against the ledger. Same-evening SELL by one sleeve and BUY by another of the same ETF go out as two orders (not netted). NSE holiday calendar still TODO (cycle aborts as stale on holidays — safe).
